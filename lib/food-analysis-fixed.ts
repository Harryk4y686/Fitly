...
export class FoodAnalysisService {
  ...
  constructor(apiKey?: string) {
    ...
    console.log('🔐 Using API key authentication for Vision API');
  }
  ...
  async analyzeFood(imageUri: string): Promise<FoodAnalysisResult> {
    ...
    try {
      // Analyze image with Vision API using API key
      ...
      // If no food detected or confidence is too low, return unknown food
      ...
      // If no matching food found in our database, return unknown food
      if (!primaryFood) {
        return this.getUnknownFoodResult(confidence);
      }
